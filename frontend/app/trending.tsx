...
export default function TrendingScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      // For trending, we could sort by some metric, or just reverse to show newest
      setSlangs(data.reverse());
    } catch (error) {
      console.error("Failed to load trending slangs", error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    >
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
            ...
            <TouchableOpacity onPress={() => router.push("/crowdsource")} style={styles.addBtn}>
        ...
        {loading && !refreshing ? (
          <View style={styles.centerContainer}>
            ...
            <Text style={styles.loadingText}>Catching the latest vibes...</Text>
          </View>
        ) : (
          <ScrollView
                    ...
                    <View key={item._id || idx} style={styles.card}>
                        <View style={styles.cardHeader}>
                            ...
                            <TouchableOpacity style={styles.heartBtn}>
                                <Ionicons name="heart-outline" size={22} color="#F06A7F" />
                            </TouchableOpacity>
                        </View>
                        ...
                        <View style={styles.tagsRow}>
                            ...
                            {item.emotionalContext && (
                                <View style={[styles.tagPill, styles.tagPillAlt]}>
                                    <Text style={[styles.tagText, styles.tagTextAlt]}>{item.emotionalContext}</Text>
                                </View>
                            )}
                        </View>
                    </View>
...
const styles = StyleSheet.create({
  ...
  backBtn: { padding: 4 },
  addBtn: { padding: 4, backgroundColor: '#FFEAF0', borderRadius: 12 },
  headerTitle: {
    fontSize: 22,
    fontWeight: '900',
    color: '#111',
